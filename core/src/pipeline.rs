...
fn handle_ipv4_packet(
    ethernet: &EthernetPacket, res_ipv4: &mut MutableIpv4Packet, destination_ip: Ipv4Addr,
    config: &AppConfig,
) -> Option<()> {
...
pub fn gdp_pipeline(
    packet: &[u8], gdp_rib: &mut RoutingInformationBase, interface: &NetworkInterface,
    tx: &mut Box<dyn DataLinkSender>, config: &AppConfig,
) -> Option<()> {
        ...
        GdpAction::Forward => {
            ...
            match tx.build_and_send(
                ...
                Some(result) => match result {
                    Ok(_) => {}
                    Err(err_msg) => {
                        println!("send error with {:?}", err_msg)
                    }
                },
                None => {
                    println!("Err: send function return none!!!");
                }
            }
        }
        GdpAction::Noop => {
