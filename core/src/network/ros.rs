...
#[cfg(feature = "ros")]
pub async fn ros_publisher_image(rib_tx: UnboundedSender<GDPPacket>, channel_tx: UnboundedSender<GDPChannel>, 
    node_name:String, topic_name:String, topic_type: String)  {
    ...
    let mut node =
        r2r::Node::create(ctx, &node_name, "namespace").expect("node creation failure");
    let publisher = node
        .create_publisher::<r2r::sensor_msgs::msg::CompressedImage>(&topic_name,  QosProfile::default())
        .expect("publisher creation failure");
